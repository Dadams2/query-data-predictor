...
# Usage: source .venv/bin/activate && bash run_decay_sweep.sh
#
...
for rate in "${RATES[@]}"; do
    ...
    query-data-predictor run-experiment -c "$config"
    ...
    if [ -n "$results_dir" ]; then
        ...
        query-data-predictor analyze-results -c "$config" -r "$results_dir"
    else
...
echo "Run: python generate_publication_figures.py  to regenerate figures"
