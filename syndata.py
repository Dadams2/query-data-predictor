...
def generate_data(config, output_dir, csv, postgres, pg_host, pg_port, 
    ...
    try:
        ...
        if postgres:
            if not pg_password:
                pg_password = click.prompt("PostgreSQL password", hide_input=True)
        ...
        # Display basic info about generated data
        print(f"\n{'='*60}")
        ...
        if postgres:
            print(f"PostgreSQL table: {pg_database}.{table_name}")
        if visualizations:
            print(f"Visualizations: plots/ directory")
        if summary:
            print(f"Summary report: dataset_summary.txt")
