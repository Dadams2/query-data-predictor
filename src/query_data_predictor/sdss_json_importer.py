...
STATEMENT_ID_COLUMN = 'statementID'

class DataLoader:
    def __init__(self, json_file_path: str, db_params: Optional[Dict[str, Any]] = None) -> None:
        """
    ...
    def _load_db_params_from_env(self) -> Dict[str, Any]:
        ...
        required_params = ['PG_SESSION_DBNAME', 'PG_SESSION_USER', 'PG_SESSION_PASSWORD', 'PG_HOST', 'PG_PORT']
    ...
    def _connect_to_db(self) -> None:
        ...
        try:
            ...
            raise ConnectionError(f"Could not connect to or create PostgreSQL database: {e}")
    ...
    def get_queries_for_session(self, session_id: str) -> np.ndarray:
        ...
        with self.conn.cursor() as cur:
            cur.execute(f"""
                SELECT {STATEMENT_ID_COLUMN}
                FROM query_data_view
                WHERE {SESSION_ID_COLUMN} = %s
                ORDER BY {STATEMENT_ID_COLUMN}
            """, (session_id,))
            result = cur.fetchall()
